...
host = os.getenv('host') if not None else "127.0.0.1"
postgres_user = os.getenv('postgres_user') if not None else "postgres"
postgres_pw = os.getenv('postgres_pw') if not None else "postgres"
project_postgres_port = os.getenv('project_postgres_port') if not None else "5432"
comment_postgres_port = os.getenv('comment_postgres_port') if not None else "5432"
...
redis_port = os.getenv('redis_port') if not None else "6379"
